...
colnames(X_mean_std_labeled) <-     gsub("Acc", "Acceleration", colnames(X_mean_std_labeled)) %>>% 
                                    gsub("Gyro", "Gyroscope", colnames(X_mean_std_labeled)) %>>% 
                                    gsub("Mag", "Magnitude", colnames(X_mean_std_labeled)) %>>% 
                                    gsub("BodyBody", "Body", colnames(X_mean_std_labeled))
...
colnames(tidy_aggregated) <-  gsub("1", "", colnames(tidy_aggregated)) %>>% 
                              gsub("\\.","", colnames(tidy_aggregated)) %>>% 
                              gsub("1", "", colnames(tidy_aggregated)) %>>% 
                              gsub("\\.", "", colnames(tidy_aggregated)) %>>%
                              gsub("mean", "Mean", colnames(tidy_aggregated)) %>>% 
                              gsub("std", "Std", colnames(tidy_aggregated))
